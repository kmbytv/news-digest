...
async function buildPrompt(articles) {
  ...
  const mainEvent = articles
    .filter(a => a.trending)
    .sort((a, b) => b.sourceCount - a.sourceCount)[0] || null;
  ...
  const lines = articles.slice(0, 40).map(a => {
    const flag = a.trending ? ` [🔥 пишут ${a.sourceCount} источников]` : '';
  ...
  const trendSection = mainEvent
    ? `🔥 Главное событие дня${NL}• [о чём пишут все источники — суть и почему важно]. → [ссылка]${NL}${NL}`
    : '';
  ...
  const fmt = [
    ...
    '',
    trendSection +
    `🤖 AI-инструменты${NL}• [суть — 2 предложения, что это и зачем тебе]. → [ссылка]`,
    '',
    `🚀 Как запускают${NL}• [история или инсайт от фаундера]. → [ссылка]`,
    '',
    `💰 Куда идут деньги${NL}• [раунд или тренд — коротко]. → [ссылка]`,
    '',
    `🌍 Экономика${NL}• [макро или рыночный тренд]. → [ссылка]`,
    '',
    `⚡ Находка дня${NL}• [что-то неожиданное но важное]`,
    '',
    `💡 Запомни сегодня: [одна простая мысль, 1-2 предложения]`,
  ].join(NL);

  const trendHint = mainEvent
    ? `Главное событие дня (об этом пишут ${mainEvent.sourceCount} источников): "${mainEvent.title}". Начни дайджест с раздела про него.`
    : 'Сегодня нет события, о котором пишут все — раздел "Главное событие дня" пропусти.';

  const prompt = [
    'Ты составляешь утренний дайджест новостей. Для большинства статей дан полный текст — используй его, чтобы объяснить суть глубоко, а не пересказывать заголовок. Статьи с 🔥 освещают несколько источников:',
    articlesText,
    trendHint,
    'Составь дайджест на русском языке. Формат:',
    fmt,
    'Правила: пиши просто, без жаргона, максимум 3 пункта на категорию, ссылка после каждого пункта. Если по категории нет новостей — пропусти её.',
  ].join(NL + NL);
...
async function main() {
  ...
  const prompt = await buildPrompt(articles);

  // ── 10. Call LLM ──
  console.log('[Step 10] Calling OpenRouter LLM...');
  let digestText;
  try {
    digestText = await callLLM(prompt);
  } catch (e) {
